...
const FinancialHealth = (props) => {
  ...
  const handleAnalyse = async () => {
    ...
    try {
      ...
      setFinancialVisualizations(
        Array.isArray(vizData?.data?.visualizations)
          ? vizData.data.visualizations
          : Array.isArray(vizData?.visualizations)
  ...
  const isButtonDisabled =
    !financialTemplate && financialReportFiles.length === 0;
